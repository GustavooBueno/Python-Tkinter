...
class View():
    def __init__(self, master, controller):
        ...
        self.buttonClientes = tk.Button(self.janela,text="Mostrar Lista")      
        self.buttonClientes.pack(side="left")
        self.buttonClientes.bind("<Button>", controller.clientesHandler) 

        #Consulta por código
        self.buttonClientes = tk.Button(self.janela,text="Consultar código")      
        self.buttonClientes.pack(side="left")
        self.buttonClientes.bind("<Button>", controller.codigoHandler) 
...
class Controller():       
    ...
    def codigoHandler(self, event):
        codigo = simpledialog.askinteger("Consulta", "Digite o código do cliente: ", parent=self.root)
        verificador = False
        for cliente in self.listaClientes: 
            if codigo == int(cliente.getCodigo()):
                self.msg = cliente.getNome() + ' - ' + cliente.getEmail() + '\n'
                self.view.mostraJanela('Cliente:', self.msg)
                verificador = True
            else:
                pass 
        if verificador == False:
            codigoErrado = 'Código não cadastrado'
            self.view.mostraJanela("Tente Novamente", codigoErrado)
        else:
            pass
